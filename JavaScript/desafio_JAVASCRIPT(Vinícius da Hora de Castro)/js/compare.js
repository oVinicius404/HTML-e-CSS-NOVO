...
function SetCarToCompare(el, carClass) {
    ...
    if (el.checked && carArr.length >= 2) {
        alert("Só é permitido comparar 2 carros");
        el.checked = false;
        return;
    ...
    if (el.checked) {
        ...
        let analise = GetCarArrPosition(carArr, carClass);
        if (analise !== -1) {
            carArr.splice(analise, 1);
        }
    }
}
...
function UpdateCompareTable() {
    let compare0 = carArr[0];
    let compare1 = carArr[1];

    document.getElementById("compare_image_0").innerHTML = `<img src="${compare0.image}" style="width: 220px;">`;
    document.getElementById("compare_image_1").innerHTML = `<img src="${compare1.image}" style="width: 220px;">`;
